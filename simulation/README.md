...
针架 CAD 和真实针尖标定不是**无工具法兰中心小步台架联调**的前置条件。该阶段使用独立的真机配置 `tool.setup: flange_only_no_tool`，并在现场核对控制器零 TCP、零负载和无附件；不能使用本仿真环境的 150 mm 临时工具参数。真正安装穿刺针后，必须重新标定法兰到针尖的刚体变换、负载和工作空间，才能另行申请针相关动作。本仿真配置始终保持 `provisional: true`、`real_robot_motion_allowed: false`。
